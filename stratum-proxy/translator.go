...
func createBlobRxEth(headerHash string, extraNonce uint32) (string, error) {
	// Remove 0x prefix if present
	headerHash = strings.TrimPrefix(headerHash, "0x")

	// CRITICAL: Validate header hash is exactly 32 bytes (64 hex chars)
	// If wrong length, blob will be invalid and shares will be rejected
	if len(headerHash) != 64 {
		return "", fmt.Errorf("invalid headerHash length: %d (expected 64 hex chars)", len(headerHash))
	}

	// Validate it's valid hex
	if _, err := hex.DecodeString(headerHash); err != nil {
		return "", fmt.Errorf("invalid headerHash hex: %w", err)
	}
...
func TargetToDifficulty(targetHex string) (uint64, error) {
	...
	if _, ok := target.SetString(targetHex, 16); !ok {
		return 0, fmt.Errorf("invalid target hex: %s", targetHex)
	}

	// Guard against zero target (would cause division by zero)
	if target.Sign() == 0 {
		return 0, fmt.Errorf("target is zero, cannot calculate difficulty")
	}
...
func DifficultyToTarget(difficulty uint64) string {
	// Guard against zero difficulty
	if difficulty == 0 {
		// Return max target (lowest difficulty)
...
// Returns (valid, actualDifficulty)
...
func ValidateShare(resultHash string, targetDiff uint64) (bool, uint64) {
	...
	hashBytes, err := hex.DecodeString(resultHash)
	if err != nil || len(hashBytes) != 32 {
		return false, 0
	}

	// CRITICAL: RandomX/Monero uses LITTLE-ENDIAN interpretation
	// Reverse the bytes to convert from little-endian to big-endian for big.Int
	reversedHash := make([]byte, 32)
	for i := 0; i < 32; i++ {
		reversedHash[i] = hashBytes[31-i]
	}

	// Parse reversed hash as big.Int (now in big-endian format)
	hashInt := new(big.Int).SetBytes(reversedHash)

	// Avoid division by zero
	if hashInt.Sign() == 0 {
		// Hash is zero - this would mean infinite difficulty
		return false, 0
	}

	// Calculate difficulty achieved: difficulty = 2^256 / hash
	// (same formula as Ethereum, but hash was read in little-endian)
	maxTarget := new(big.Int).Exp(big.NewInt(2), big.NewInt(256), nil)
	achievedDiff := new(big.Int).Div(maxTarget, hashInt)

	// Compare with target difficulty
	targetDiffBig := new(big.Int).SetUint64(targetDiff)
	valid := achievedDiff.Cmp(targetDiffBig) >= 0

	// For return value, convert to uint64 (but validation used big.Int)
	// If achievedDiff > 2^64-1, cap at max uint64 for display purposes
	var actualDiff uint64
	if achievedDiff.BitLen() > 64 {
		actualDiff = ^uint64(0) // Max uint64
	} else {
